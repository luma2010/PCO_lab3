...
void Utils::endService() {
  std::for_each(threads.begin(), threads.end(),
                [](auto &thread) { thread->requestStop(); });
}
...
std::vector<Ambulance *> createAmbulances(int nbAmbulances, int idStart) {
  ...
  for (int i = 0; i < nbAmbulances; ++i) {
    ...
    case 0: {
      std::map<ItemType, int> initialAmbulanceStock = {
          {ItemType::PatientSick, INITIAL_PATIENT_SICK}};
      ambulances.push_back(new Ambulance(i + idStart, SUPPLIER_FUND,
...
std::vector<Supplier *> createSuppliers(int nbSuppliers, int idStart) {
  ...
  for (int i = 0; i < nbSuppliers; ++i) {
    ...
    case 1: {
      suppliers.push_back(
          new MedicalDeviceSupplier(i + idStart, SUPPLIER_FUND));
      break;
    }
    case 2: {
...
std::vector<Hospital *> createHospitals(int nbHospital, int idStart) {
  ...
  for (int i = 0; i < nbHospital; ++i) {
    hospitals.push_back(
        new Hospital(i + idStart, HOSPITALS_FUND, MAX_BEDS_PER_HOSTPITAL));
  }
...
Utils::Utils(int nbSupplier, int nbClinic, int nbHospital) {
  ...
  for (auto &h : hospitals) {
    std::vector<Seller *> tmpClinics(clinics.begin() + countClinic,
                                     clinics.begin() + countClinic +
                                         clinicsByHospital);
    tmpClinics.insert(tmpClinics.end(), clinics.end() - clinicsShared,
                      clinics.end());
...
void Utils::run() {
  ...
  for (size_t i = 0; i < ambulances.size(); ++i) {
    threads.emplace_back(
        std::make_unique<PcoThread>(&Ambulance::run, ambulances[i]));
  }

  for (size_t i = 0; i < suppliers.size(); ++i) {
    threads.emplace_back(
        std::make_unique<PcoThread>(&Supplier::run, suppliers[i]));
  }
  ...
  for (size_t i = 0; i < hospitals.size(); ++i) {
    threads.emplace_back(
        std::make_unique<PcoThread>(&Hospital::run, hospitals[i]));
  }
  ...
  finalReport =
      QString("The expected fund is : %1 and you got at the end : %2\n")
          .arg(startFund)
          .arg(endFund);
  finalReport +=
      QString("The expected patient is : %1 and you got at the end : %2")
          .arg(startPatient)
          .arg(endPatient);
...
QString Utils::getFinalReport() { return finalReport; }
