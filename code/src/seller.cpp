...
Seller *Seller::chooseRandomSeller(std::vector<Seller *> &sellers) {
  ...
  std::sample(sellers.begin(), sellers.end(), std::back_inserter(out), 1,
...
int getCostPerUnit(ItemType item) {
  ...
  case ItemType::Syringe:
    return SYRINGUE_COST;
  case ItemType::Pill:
    return PILL_COST;
  case ItemType::Scalpel:
    return SCALPEL_COST;
  case ItemType::Thermometer:
    return THERMOMETER_COST;
  case ItemType::Stethoscope:
    return STETHOSCOPE_COST;
  case ItemType::PatientHealed:
    return HEALING_COST;
