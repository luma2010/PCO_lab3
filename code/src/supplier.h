...
public:
   ...
   * @return Les items dans les stocks à vendre sous forme d'une map (clé : type
   * d'item, valeur : quantité)
   */
  ...
  /**
   * @brief Gérer l'opération du fournisseur, mise à jour des stocks et paiement
   * des employés Cette fonction gère l'augmentation des stocks, les
   * transactions, ainsi que la gestion des employés.
   */
  void run();
   ...
   * afficher les logs et mises à jour
   */
...
protected:
  std::vector<ItemType>
      resourcesSupplied; // Liste des items que ce fournisseur gère
